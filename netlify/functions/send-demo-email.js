...
exports.handler = async (event) => {
  ...
  const { name, email, company, industry, preferred_date } = body;
  ...
  if (!company || !company.trim()) {
    return { statusCode: 400, headers: HEADERS, body: JSON.stringify({ error: 'Please provide your company name.' }) };
  }
  if (!industry) {
    return { statusCode: 400, headers: HEADERS, body: JSON.stringify({ error: 'Please select your industry.' }) };
  }
