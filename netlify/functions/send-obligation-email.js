...
export const handler = async (event) => {
  ...
  const { name, business, email, phone, businessType, otherInfo } = JSON.parse(event.body || '{}');
  ...
  try {
    ...
    return { statusCode: 500, headers: HEADERS, body: JSON.stringify({ error: 'Failed to send email.' }) };
  }
};
